...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (batchId && typeof batchId === "string") {
    try {
      ...
      console.warn("[community] Failed to fetch batch details");
    }
  }
  ...
  for (const tokenObj of activeTokens) {
    ...
    const currentRandomId = tokenObj.randomId || "";

    try {
      const url = `${PW_COMMUNITY_API}/v3/community/posts/v2?channelId=${channelId}&page=${page}&timestamp=${timestamp}`;
      ...
      if (response.ok) {
        ...
        console.warn(`[community] Token ${tokenObj.randomId} returned status ${response.status}: ${errText}`);
        if (response.status === 401) {
          try {
            ...
            console.log(`[community] Deactivated expired token ${tokenObj.randomId} in DB.`);
          } catch (dbErr: any) {
            console.error("[community] Failed to deactivate expired token:", dbErr.message);
          }
        }
        finalStatus = response.status;
        finalMessage = "Failed to fetch community posts.";
      }
    } catch (fetchErr: any) {
      console.warn(`[community] Fetch failed with token ${tokenObj.randomId}: ${fetchErr.message}`);
      finalStatus = 500;
